...
class JobsModel(db.Model):
    ...
    @classmethod
    def find_by_time(cls, time, page, limit):
       
        jobs = cls.query.filter(func.lower(cls.date_extracted).contains(func.lower(time))).paginate(page=page, per_page=limit, max_per_page=limit)
        if jobs:
            return jobs
        return None
